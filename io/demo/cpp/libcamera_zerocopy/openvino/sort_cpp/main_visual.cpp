...
int main(int argc, char* argv[])
{
    ...
    while (cap.read(frame))
    {
        ...
        for (const auto& obj : tracked_objects)
        {
            ...
            std::string label = "ID: " + std::to_string(obj.id) + " Class: " + std::to_string(obj.class_id);
            cv::putText(frame, label, cv::Point(obj.box.x, obj.box.y - 10), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 255, 0), 2);
        }

        cv::imshow("YOLOv5 Object Tracking", frame);
        if (cv::waitKey(1) == 'q')
        {
            break;
        }
