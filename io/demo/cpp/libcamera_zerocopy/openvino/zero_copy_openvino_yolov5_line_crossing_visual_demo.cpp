...
struct LineCrossingZone {
    ...
    Point end;
    std::string name;
    cv::Scalar color;  // 시각화를 위한 색상 추가
    
    LineCrossingZone(const Point& s, const Point& e, const std::string& n, const cv::Scalar& c = cv::Scalar(0, 255, 0)) 
        : start(s), end(e), name(n), color(c) {}
};
...
struct CrossingEvent {
    ...
    CrossingEvent(int id, const std::string& zone, const Point& point)
        : track_id(id), zone_name(zone), timestamp(std::chrono::steady_clock::now()), crossing_point(point) {}
};
...
public:
    ...
    const std::vector<LineCrossingZone>& getZones() const {
        return zones;
    }
...
public:
    ...
    // 시각화 함수 추가
    void drawVisualization(cv::Mat& display_frame) {
        // 1. Line crossing zones 그리기
        const auto& zones = line_detector.getZones();
        for (const auto& zone : zones) {
            cv::Point start_cv(static_cast<int>(zone.start.x), static_cast<int>(zone.start.y));
            cv::Point end_cv(static_cast<int>(zone.end.x), static_cast<int>(zone.end.y));
            
            // 라인 그리기 (두꺼운 선)
            cv::line(display_frame, start_cv, end_cv, zone.color, 3);
            
            // 라인 이름 표시
            cv::Point text_pos(start_cv.x + 10, start_cv.y - 10);
            cv::putText(display_frame, zone.name, text_pos, cv::FONT_HERSHEY_SIMPLEX, 0.6, zone.color, 2);
        }
        
        // 2. Bounding boxes와 track ID 그리기
        for (const auto& track : tracks) {
            // 바운딩 박스 그리기
            cv::Scalar bbox_color(0, 255, 255); // 노란색
            cv::rectangle(display_frame, track.bbox, bbox_color, 2);
            
            // Track ID와 신뢰도 표시
            std::string label = "ID:" + std::to_string(track.id) + " (" + 
                               std::to_string(static_cast<int>(track.confidence * 100)) + "%)";
            cv::Point text_pos(track.bbox.x, track.bbox.y - 10);
            ...
            // 텍스트 그리기
            cv::putText(display_frame, label, text_pos, cv::FONT_HERSHEY_SIMPLEX, 0.5, 
                       cv::Scalar(255, 255, 255), 1);
            
            // 중심점 그리기
            Point center = line_detector.getBboxCenter(track.bbox);
            cv::Point center_cv(static_cast<int>(center.x), static_cast<int>(center.y));
            cv::circle(display_frame, center_cv, 3, cv::Scalar(255, 0, 0), -1); // 파란색 점
        }
        
        // 3. 최근 crossing 이벤트 표시
        const auto& recent_crossings = line_detector.getRecentCrossings();
        for (size_t i = 0; i < recent_crossings.size() && i < 5; ++i) { // 최대 5개만 표시
            const auto& event = recent_crossings[recent_crossings.size() - 1 - i];
            cv::Point crossing_pt(static_cast<int>(event.crossing_point.x), 
                                 static_cast<int>(event.crossing_point.y));
            
            // Crossing 포인트에 빨간 원 그리기
            cv::circle(display_frame, crossing_pt, 8, cv::Scalar(0, 0, 255), 3);
            
            // Crossing 정보 텍스트
            std::string crossing_text = "CROSS! ID:" + std::to_string(event.track_id);
            cv::Point crossing_text_pos(crossing_pt.x + 15, crossing_pt.y);
            cv::putText(display_frame, crossing_text, crossing_text_pos, 
                       cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(0, 0, 255), 1);
        }
        
        // 4. 상태 정보 표시
        std::string status_text = "Tracks: " + std::to_string(tracks.size()) + 
                                 " | Crossings: " + std::to_string(recent_crossings.size());
        cv::putText(display_frame, status_text, cv::Point(10, 30), 
...
private:
    ...
    void setupDefaultLineCrossingZones() {
        // 1920x1080 해상도 기준으로 line crossing zone 설정 (다양한 색상)
        // 화면 중앙 가로선 (녹색)
        ...
        line_detector.addZone(LineCrossingZone(
            ...
            "lower_third",
            cv::Scalar(255, 0, 255)  // 자홍색
        ));
        
        std::cout << "Line Crossing Zones 설정 완료:" << std::endl;
        std::cout << "  - center_horizontal: (0,540) - (1920,540) [녹색]" << std::endl;
        std::cout << "  - upper_third: (0,360) - (1920,360) [파란색]" << std::endl;
        std::cout << "  - lower_third: (0,720) - (1920,720) [자홍색]" << std::endl;
    }
...
class ZeroCopyOpenVINOYOLOv5LineCrossingVisualTracker {
...
public:
    ZeroCopyOpenVINOYOLOv5LineCrossingVisualTracker(const std::string& model_xml) {
    ...
    bool start() {
        ...
        camera->requestCompleted.connect(this, &ZeroCopyOpenVINOYOLOv5LineCrossingVisualTracker::onRequestCompleted);
        if (camera->start()) {
        ...
        std::cout << "캡처 및 OpenVINO YOLOv5 + SORT + Line Crossing + 시각화 시작..." << std::endl;
        return true;
    }
    ...
    void stop() {
        ...
        if (camera) {
            camera->stop();
            camera->requestCompleted.disconnect(this, &ZeroCopyOpenVINOYOLOv5LineCrossingVisualTracker::onRequestCompleted);
        }
    ...
    void onRequestCompleted(libcamera::Request* request) {
        ...
        // Zero-Copy를 유지하면서 시각화를 위한 복사본 생성 (display용만)
        cv::Mat display_frame = frame.clone();
        
        // 시각화 그리기
        yolo_tracker->drawVisualization(display_frame);
        
        // OpenCV 창에 표시
        cv::imshow("YOLOv5 + SORT + Line Crossing", display_frame);
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(now - lastTime).count() >= 1) {
            ...
            std::cout << "[YOLOv5+SORT+LineCrossing+Visual FPS: " << std::fixed << std::setprecision(1) << fps 
...
static ZeroCopyOpenVINOYOLOv5LineCrossingVisualTracker* demoInstance = nullptr;
...
int main(int argc, char** argv) {
    std::cout << "=== Zero Copy OpenVINO YOLOv5 + SORT + Line Crossing + 시각화 Demo ===" << std::endl;
    ...
    std::cout << "OpenCV 시각화 활성화 - ESC 키로 종료 가능" << std::endl;
    
    signal(SIGINT, signalHandler);
    signal(SIGTERM, signalHandler);

    ZeroCopyOpenVINOYOLOv5LineCrossingVisualTracker demo(model_xml);
    demoInstance = &demo;
    ...
    std::cout << "YOLOv5 + SORT + Line Crossing + 시각화 실행 중... (Ctrl+C 또는 ESC로 종료)" << std::endl;
