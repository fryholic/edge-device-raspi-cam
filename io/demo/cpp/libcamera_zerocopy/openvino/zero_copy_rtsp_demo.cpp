...
//
// 수정 사항 (v4 - 파이프라인 안정화):
// 1. GStreamer 파이프라인 수정: videoconvert의 출력 포맷을 NV12로 명시하고, queue 엘리먼트를 추가하여 데이터 흐름 안정성 확보
// 2. 스레드 안전 큐 개선: Condition Variable을 사용한 blocking pop 방식으로 변경하여 CPU 효율성 증대
// 3. 종료 로직 개선: 프로그램 종료 시 처리 스레드가 즉시 반응하도록 수정
//
// 빌드 방법:
// make rtsp-demo
//
...
public:
    ...
    bool start() {
        ...
        // ** GStreamer 파이프라인: queue 추가 및 videoconvert 포맷 명시 **
        std::string pipeline_str = "appsrc name=mysrc ! queue ! videoconvert ! video/x-raw,format=NV12 ! v4l2h264enc ! video/x-h264,level=(string)4 ! rtph264pay name=pay0 pt=96";
...
public:
    ...
    void stop() {
        ...
        frame_queue.stop(); // 큐를 중지시키고 대기 중인 스레드를 깨움
    ...
    void processFrames() {
        while (!stopping.load()) {
            ...
            if (frame_queue.wait_and_pop(frame)) {
                rtsp_server->pushFrame(frame.data, frame.size);
                frameCount++;
...
int main() {
    ...
    std::cout << "========================================================" << std::endl;
    std::cout << "   Zero-Copy Camera to RTSP Streamer (v4 - Stabilized)" << std::endl;
    std::cout << "========================================================" << std::endl;
