...
private:
    ...
    void init_device() {
        ...
        // 1920x1080 해상도 우선 시도, 실패 시 fallback 로직
        ...
        // 1920x1080 YUYV 포맷 우선 시도 (OpenCV에서 쉽게 변환 가능)
        ...
        if (ioctl(fd_, VIDIOC_S_FMT, &fmt) == -1) {
            std::cerr << "1920x1080 YUYV failed, trying 1920x1080 H.264: " << strerror(errno) << std::endl;
            // 1920x1080 YUYV 실패 시 1920x1080 H.264로 시도
            fmt.fmt.pix.pixelformat = v4l2_fourcc('H', '2', '6', '4');
            fmt.fmt.pix.sizeimage = 1920 * 1080 * 2;
            
            if (ioctl(fd_, VIDIOC_S_FMT, &fmt) == -1) {
                std::cerr << "1920x1080 H.264 also failed, falling back to 640x480: " << strerror(errno) << std::endl;
                
                // 640x480 H.264로 fallback
                fmt.fmt.pix.width = 640;
                fmt.fmt.pix.height = 480;
                fmt.fmt.pix.pixelformat = v4l2_fourcc('H', '2', '6', '4');
                fmt.fmt.pix.sizeimage = 640 * 480 * 2;
                
                std::cout << "Trying to set 640x480 resolution with H.264 format..." << std::endl;
                
                if (ioctl(fd_, VIDIOC_S_FMT, &fmt) == -1) {
                    std::cerr << "640x480 H.264 format failed, falling back to YUYV: " << strerror(errno) << std::endl;
                    // 640x480 H.264 실패 시 640x480 YUYV로 폴백
                    fmt.fmt.pix.pixelformat = v4l2_fourcc('Y', 'U', 'Y', 'V');
                    fmt.fmt.pix.sizeimage = fmt.fmt.pix.width * fmt.fmt.pix.height * 2;  // YUYV 크기
                    
                    if (ioctl(fd_, VIDIOC_S_FMT, &fmt) == -1) {
                        std::cerr << "640x480 YUYV format also failed: " << strerror(errno) << std::endl;
                        // 실패 시 현재 설정으로 계속 진행
                        if (ioctl(fd_, VIDIOC_G_FMT, &fmt) == -1) {
                    ...
                    std::cout << "Successfully set to 640x480 with H.264 (fallback)!" << std::endl;
                    format_set = true;
                }
            } else {
...
// OpenCV를 사용하여 프레임을 화면에 표시하는 함수
cv::Mat convert_frame_to_mat(const CapturedFrame& frame, int width, int height, uint32_t format) {
    cv::Mat image;
    
    if (format == v4l2_fourcc('Y', 'U', 'Y', 'V')) {
        // YUYV를 BGR로 변환
        cv::Mat yuyv_image(height, width, CV_8UC2, frame.data);
        cv::cvtColor(yuyv_image, image, cv::COLOR_YUV2BGR_YUYV);
    } else if (format == v4l2_fourcc('Y', 'U', '1', '2')) {
        // YUV420 (I420)을 BGR로 변환
        cv::Mat yuv_image(height * 3 / 2, width, CV_8UC1, frame.data);
        cv::cvtColor(yuv_image, image, cv::COLOR_YUV420p2BGR);
    } else if (format == v4l2_fourcc('M', 'J', 'P', 'G') || format == v4l2_fourcc('J', 'P', 'E', 'G')) {
        // MJPEG 디코딩
        std::vector<uchar> jpeg_data((uchar*)frame.data, (uchar*)frame.data + frame.size);
        image = cv::imdecode(jpeg_data, cv::IMREAD_COLOR);
        if (image.empty()) {
            std::cerr << "Failed to decode MJPEG frame" << std::endl;
            return cv::Mat::zeros(height, width, CV_8UC3);
        }
    } else {
        // 다른 포맷의 경우 빈 이미지 반환 (또는 raw 데이터로 처리)
        std::cerr << "Unsupported format for display: 0x" << std::hex << format << std::dec << std::endl;
        image = cv::Mat::zeros(height, width, CV_8UC3);
        
        // 포맷 정보를 이미지에 텍스트로 표시
        char format_str[5];
        format_str[0] = format & 0xFF;
        format_str[1] = (format >> 8) & 0xFF;
        format_str[2] = (format >> 16) & 0xFF;
        format_str[3] = (format >> 24) & 0xFF;
        format_str[4] = '\0';
        
        std::string text = "Format: " + std::string(format_str);
        cv::putText(image, text, cv::Point(30, 50), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255, 255, 255), 2);
        cv::putText(image, "Not supported for display", cv::Point(30, 100), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255, 255, 255), 2);
    }
...
int main() {
    ...
    std::cout << "Starting V4L2 Camera Test with OpenCV Display..." << std::endl;
    ...
    try {
        ...
        if (is_h264) {
            std::cout << "⚠ Using H.264 format (cannot display directly)" << std::endl;
        } else if (is_yuv420) {
            std::cout << "✓ Using YUV420 format (displayable)" << std::endl;
        } else if (is_mjpeg) {
            std::cout << "✓ Using MJPEG format (displayable)" << std::endl;
        } else if (is_yuyv) {
            std::cout << "✓ Using YUYV format (displayable)" << std::endl;
        } else {
        ...
        std::cout << "Camera streaming started. Starting display..." << std::endl;
        std::cout << "Press 'q' to quit, 's' to save frame" << std::endl;
        
        // OpenCV 창 생성
        cv::namedWindow("V4L2 Camera Feed", cv::WINDOW_AUTOSIZE);
        
        // FPS 측정을 위한 변수들
        auto start_time = std::chrono::high_resolution_clock::now();
        int total_frames = 0;
        int saved_frames = 0;
        const int timing_frames = 30;
        ...
        while (true) {
            try {
                ...
                // OpenCV Mat으로 변환하여 화면에 표시
                cv::Mat image = convert_frame_to_mat(frame, width, height, format);
                ...
                if (!image.empty()) {
                    ...
                    // 화면에 표시
                    cv::imshow("V4L2 Camera Feed", image);
                }
                
                // H.264 프레임 저장 (처음 5개)
                if (is_h264 && saved_frames < 5) {
                ...
                if (key == 'q' || key == 27) {  // 'q' 또는 ESC
                    ...
                    break;
                } else if (key == 's') {  // 's'로 프레임 저장
                    if (!is_h264) {
                        std::string filename = "saved_frame_" + std::to_string(saved_frames) + ".jpg";
                        cv::Mat save_image = convert_frame_to_mat(frame, width, height, format);
                        if (!save_image.empty()) {
