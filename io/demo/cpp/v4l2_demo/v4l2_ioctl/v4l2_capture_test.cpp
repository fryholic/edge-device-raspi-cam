...
CapturedFrame V4L2Camera::capture_frame(bool show_timing) {
    ...
    // 먼저 non-blocking으로 DQBUF를 직접 시도 (select 건너뛰기)
    struct v4l2_buffer buf = {};
    buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    buf.memory = V4L2_MEMORY_MMAP;
    ...
    if (ioctl(fd_, VIDIOC_DQBUF, &buf) == 0) {
        ...
        if (show_timing) {
            ...
            std::cout << "Timing: direct VIDIOC_DQBUF took " << total_duration << " us (no select needed)." << std::endl;
        }
        return {buffers_[buf.index].start, buf.bytesused, (int)buf.index, show_timing};
    }

    // DQBUF가 실패한 경우에만 select 사용
    if (errno != EAGAIN) {
        throw std::runtime_error("Failed to dequeue buffer (VIDIOC_DQBUF)");
    }

    // 짧은 timeout으로 select 사용
    fd_set fds;
    FD_ZERO(&fds);
    FD_SET(fd_, &fds);
    ...
    if (r == 0) {
        throw std::runtime_error("Frame capture timeout");
    }
    ...
    if (ioctl(fd_, VIDIOC_DQBUF, &buf) == -1) {
        throw std::runtime_error("Failed to dequeue buffer after select");
    }

    auto dqbuf_end_time = std::chrono::high_resolution_clock::now();

    if (show_timing) {
        auto select_duration = std::chrono::duration_cast<std::chrono::microseconds>(select_end_time - capture_start_time).count();
        auto dqbuf_duration = std::chrono::duration_cast<std::chrono::microseconds>(dqbuf_end_time - select_end_time).count();
        std::cout << "Timing: select() took " << select_duration << " us, VIDIOC_DQBUF took " << dqbuf_duration << " us." << std::endl;
    }
...
int main() {
    ...
    std::cout << "Starting V4L2 Camera Test with optimizations..." << std::endl;
    ...
    try {
        ...
        std::cout << "Camera streaming started. Starting FPS measurement (no file saving)..." << std::endl;
        std::cout.flush();
